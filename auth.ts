...
export const {handlers,auth,signIn,signOut}=NextAuth({
  ...
  providers:[
    ...(configuration.google?[Google({clientId:process.env.AUTH_GOOGLE_ID,clientSecret:process.env.AUTH_GOOGLE_SECRET,allowDangerousEmailAccountLinking:false})]:[]),
    Credentials({id:"demo",name:"Demo",credentials:{role:{}},authorize:async credentials=>{
      if(!configuration.demoEnabled)return null;
      const role=credentials?.role as Role;
      if(!ROLES.includes(role))return null;
      await connectDb();
      const email=`demo.${role.toLowerCase()}@careshield.invalid`;
      const user=await User.findOneAndUpdate({email},{email,name:`Demo ${role.replace("_"," ")}`,role,accountState:"ACTIVE",onboardingComplete:true,organizationIds:[]},{upsert:true,new:true,setDefaultsOnInsert:true});
